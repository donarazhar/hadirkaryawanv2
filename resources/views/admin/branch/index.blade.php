...
@section('title', 'Lokasi Presensi Cabang')
@section('page-title', 'Lokasi Presensi Cabang')
...
<style>
    ...
    .cabang-wrap { display: flex; flex-direction: column; gap: 20px; }
    ...
    .cabang-header-left { display: flex; align-items: center; gap: 14px; }
    ...
    .cabang-header-icon i { font-size: 22px; color: var(--blue); }

    .cabang-header-title { font-size: 17px; font-weight: 800; color: var(--slate-900); letter-spacing: -0.2px; }
    .cabang-header-sub   { font-size: 12px; color: var(--slate-400); margin-top: 2px; }
    ...
    .cabang-name { font-size: 13.5px; font-weight: 800; color: var(--slate-900); display: flex; align-items: center; gap: 8px; }
    .cabang-name i { color: var(--blue); font-size: 16px; }

    .loc-text { font-size: 12.5px; color: var(--slate-600); display: flex; align-items: flex-start; gap: 6px; line-height: 1.4; }
    .loc-text i { color: var(--slate-400); font-size: 15px; margin-top: 1px; }

    .rad-pill { display: inline-flex; align-items: center; gap: 4px; background: var(--slate-100); color: var(--slate-700); padding: 4px 10px; border-radius: 50px; font-size: 11.5px; font-weight: 700; border: 1px solid var(--slate-200); }
    .rad-pill i { font-size: 14px; color: var(--slate-500); }
...
@section('content')
<div class="cabang-wrap">

    {{-- HEADER --}}
    <div class="cabang-header">
        <div class="cabang-header-left">
            <div class="cabang-header-icon">
                <i class="mdi mdi-domain"></i>
            </div>
            <div>
                <div class="cabang-header-title">Master Data Cabang (Lokasi Presensi)</div>
                <div class="cabang-header-sub">Kelola data lokasi dan radius absen cabang</div>
            </div>
        </div>
        @if(Auth::guard('user')->user()->role === 'superadmin')
        <div>
            <a href="{{ route('panel.branch.create') }}" class="btn-hdr btn-hdr-primary">
    ...
    <div class="filter-card">
        ...
        <div class="filter-body">
            <form action="{{ route('panel.branch.index') }}" method="GET">
                <div class="filter-grid">
                    <div class="fg">
                        <label>Cari Cabang</label>
                        <input type="text" name="search" placeholder="Cari berdasarkan nama atau lokasi..." value="{{ request('search') }}">
                    </div>
                    <div>
    ...
    <div class="tbl-card">
        <div class="tbl-card-head">
            <div class="tbl-card-title"><i class="mdi mdi-format-list-bulleted"></i> Daftar Cabang</div>
            <span class="tbl-meta">{{ $branches->total() }} Cabang</span>
        </div>
        <div class="tbl-wrap">
            <table>
                <thead>
                    <tr>
                        ...
                        <th>Nama Cabang</th>
                        <th>Titik Lokasi</th>
                        <th style="width:120px;">Radius Absen</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="no-cell">{{ $branches->firstItem() + $index }}</td>
                        <td>
                            <div class="cabang-name">
                        ...
                        <td>
                            <div class="loc-text">
                                ...
                                <span>{{ $item->lokasi_cabang ?? 'Belum di set' }}</span>
                            </div>
                        </td>
                        <td>
                            <div class="rad-pill">
                                <i class="mdi mdi-radar"></i>
                                {{ number_format($item->radius_cabang) }} m
                            </div>
                        </td>
                        <td>
                            <div class="action-cell">
                                <a href="{{ route('panel.branch.cetakQr', $item->id) }}" target="_blank" class="btn-act btn-act-blue" title="Cetak QR Code">
                                    <i class="mdi mdi-qrcode"></i>
                                </a>
                                <a href="{{ route('panel.branch.edit', $item->id) }}" class="btn-act btn-act-yellow" title="Edit">
                                    <i class="mdi mdi-pencil"></i>
                                </a>
                                @if(Auth::guard('user')->user()->role === 'superadmin')
                                <form action="{{ route('panel.branch.destroy', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus cabang ini? Semua data terkait (Unit, Organ) mungkin akan terpengaruh.');">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="pag-wrap">
            <div class="pag-info">
                Menampilkan <strong>{{ $branches->firstItem() ?? 0 }}</strong>–<strong>{{ $branches->lastItem() ?? 0 }}</strong> dari <strong>{{ $branches->total() }}</strong> cabang
            </div>
            <ul class="pag-list">
                @if($branches->onFirstPage())
                    <li class="pag-item disabled"><span><i class="mdi mdi-chevron-left"></i></span></li>
                @else
                    <li class="pag-item"><a href="{{ $branches->previousPageUrl() }}"><i class="mdi mdi-chevron-left"></i></a></li>
                @endif

                @foreach(range(1, $branches->lastPage()) as $i)
                    @if($i == $branches->currentPage())
                        <li class="pag-item active"><span>{{ $i }}</span></li>
                    @else
                        <li class="pag-item"><a href="{{ $branches->url($i) }}">{{ $i }}</a></li>
                    @endif
                @endforeach

                @if($branches->hasMorePages())
                    <li class="pag-item"><a href="{{ $branches->nextPageUrl() }}"><i class="mdi mdi-chevron-right"></i></a></li>
                @else
