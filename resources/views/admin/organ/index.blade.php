...
@section('title', 'Data Organ / Jabatan')
@section('page-title', 'Data Organ / Jabatan')
...
<style>
    ...
    .organ-wrap { display: flex; flex-direction: column; gap: 20px; }
    ...
    .organ-header-left { display: flex; align-items: center; gap: 14px; }
    ...
    .organ-header-icon i { font-size: 22px; color: var(--blue); }

    .organ-header-title { font-size: 17px; font-weight: 800; color: var(--slate-900); letter-spacing: -0.2px; }
    .organ-header-sub   { font-size: 12px; color: var(--slate-400); margin-top: 2px; }
    ...
    .organ-name { font-size: 13.5px; font-weight: 800; color: var(--slate-900); display: flex; align-items: center; gap: 8px; }
    .organ-name i { color: var(--blue); font-size: 16px; }
    
    .rel-text { font-size: 12px; color: var(--slate-600); line-height: 1.4; display: flex; flex-direction: column; gap: 2px; }
    .rel-text strong { color: var(--slate-800); }
...
<div class="organ-wrap">
    ...
    <div class="organ-header">
        <div class="organ-header-left">
            <div class="organ-header-icon">
                <i class="mdi mdi-account-tie"></i>
            </div>
            <div>
                <div class="organ-header-title">Master Data Organ (Jabatan)</div>
                <div class="organ-header-sub">Kelola hierarki jabatan di bawah struktur Unit</div>
            </div>
        </div>
        <div>
            <a href="{{ route('panel.organ.create') }}" class="btn-hdr btn-hdr-primary">
                <i class="mdi mdi-plus"></i> Tambah Organ
            </a>
        </div>
    </div>
    ...
    <div class="filter-card">
        ...
        <div class="filter-body">
            <form action="{{ route('panel.organ.index') }}" method="GET">
                <div class="filter-grid">
                    <div class="fg">
                        <label>Cari Organ</label>
                        <input type="text" name="search" placeholder="Cari berdasarkan nama organ..." value="{{ request('search') }}">
                    </div>
                    <div>
    ...
    <div class="tbl-card">
        <div class="tbl-card-head">
            <div class="tbl-card-title"><i class="mdi mdi-format-list-bulleted"></i> Daftar Organ</div>
            <span class="tbl-meta">{{ $organs->total() }} Organ</span>
        </div>
        <div class="tbl-wrap">
            <table>
                <thead>
                ...
                <tbody>
                    @forelse($organs as $index => $item)
                    <tr>
                        <td class="no-cell">{{ $organs->firstItem() + $index }}</td>
                        <td>
                        ...
                        <td>
                            <span class="badge bg-primary" style="font-weight: 600; padding: 5px 10px; color: #fff;">
                                {{ $item->unit->name ?? '-' }}
                            </span>
                        </td>
                        <td>
                            <div class="rel-text">
                                <span>{{ $item->unit->branch->name ?? '-' }}</span>
                            </div>
                        </td>
                        <td>
                            <div class="action-cell">
                                <a href="{{ route('panel.organ.edit', $item->id) }}" class="btn-act btn-act-yellow" title="Edit">
                                    <i class="mdi mdi-pencil"></i>
                                </a>
                                <form action="{{ route('panel.organ.destroy', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus organ ini?');">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="pag-wrap">
            <div class="pag-info">
                Menampilkan <strong>{{ $organs->firstItem() ?? 0 }}</strong>–<strong>{{ $organs->lastItem() ?? 0 }}</strong> dari <strong>{{ $organs->total() }}</strong> organ
            </div>
            <ul class="pag-list">
                @if($organs->onFirstPage())
                    <li class="pag-item disabled"><span><i class="mdi mdi-chevron-left"></i></span></li>
                @else
                    <li class="pag-item"><a href="{{ $organs->previousPageUrl() }}"><i class="mdi mdi-chevron-left"></i></a></li>
                @endif

                @foreach(range(1, $organs->lastPage()) as $i)
                    @if($i == $organs->currentPage())
                        <li class="pag-item active"><span>{{ $i }}</span></li>
                    @else
                        <li class="pag-item"><a href="{{ $organs->url($i) }}">{{ $i }}</a></li>
                    @endif
                @endforeach

                @if($organs->hasMorePages())
                    <li class="pag-item"><a href="{{ $organs->nextPageUrl() }}"><i class="mdi mdi-chevron-right"></i></a></li>
                @else
