...
<div class="form-wrap">
    ...
    <div class="form-header">
        <div class="form-header-left">
            ...
            <div>
                <div class="form-header-title">Edit Pengguna</div>
                <div class="form-header-sub">Ubah profil atau tingkat akses role pengguna</div>
            </div>
        </div>
        <div>
    ...
    <div class="form-grid">
        ...
        {{-- LEFT COLUMN: FORM --}}
        <div>
            <form action="{{ route('panel.user.update', $user->id) }}" method="POST" id="formUser">
                @csrf
                @method('PUT')
                    ...
                    <div class="card-body">
                        ...
                        <div class="fg">
                            ...
                            <select name="role" id="role-select" class="form-select @error('role') is-invalid @enderror" required>
                                ...
                                <option value="pimpinan" {{ old('role', $user->role) == 'pimpinan' ? 'selected' : '' }}>Pimpinan Departemen</option>
                            </select>
                            @error('role')
                        ...
                        <div class="fg" id="karyawan-container" style="display:none;">
                            ...
                            <div class="form-hint"><i class="mdi mdi-information-outline"></i> Pilih Karyawan hanya jika ingin mengubah isian profil secara otomatis sesuai data karyawan.</div>
                        </div>
                            ...
                            <div class="fg" id="cabang-container" style="display:none; margin-bottom:0;">
                                ...
                                <select name="kode_cabang" id="cabang-select" class="form-select @error('kode_cabang') is-invalid @enderror">
                                    ...
                                    @foreach($cabang as $item)
                                        <option value="{{ $item->kode_cabang }}" {{ old('kode_cabang', $user->kode_cabang) == $item->kode_cabang ? 'selected' : '' }}>
                            ...
                            <div class="fg" id="dept-container" style="display:none; margin-bottom:0;">
                                ...
                                <select name="kode_dept" id="dept-select" class="form-select @error('kode_dept') is-invalid @enderror">
                                    ...
                                    @foreach($departemen as $item)
                                        <option value="{{ $item->kode_dept }}" {{ old('kode_dept', $user->kode_dept) == $item->kode_dept ? 'selected' : '' }}>
                        ...
                        <div class="fg-row" style="margin-bottom:0;">
                            <div class="fg" style="margin-bottom:0;">
                                ...
                                <input type="email" id="email-input" name="email" class="form-control @error('email') is-invalid @enderror" 
                                    value="{{ old('email', $user->email) }}" placeholder="Contoh: budi@perusahaan.com" required>
                                @error('email')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="fg" id="password-container" style="margin-bottom:0;">
                                ...
                                <div class="form-hint"><i class="mdi mdi-information-outline"></i> Kosongkan jika tidak ingin mengubah sandi.</div>
                                @error('password')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
            ...
            <div class="card">
                <div class="card-head">
                    <h3 class="card-title" style="display:flex;align-items:center;gap:8px;">
                        <i class="mdi mdi-shield-check" style="color:var(--amber);font-size:18px;"></i>
                        Tingkat Akses Role
                    </h3>
                </div>
                <div class="card-body">
